...
class Console
  ...
  def create_account
    loop do
      account = Builder.build do |builder|
        builder.input_name(input(:name))
        builder.input_age(input(:age))
        builder.input_login_credentials(input(:login), input(:password))
        builder.input_empty_cards_list
      end
      next unless account

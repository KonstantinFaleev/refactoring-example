class Console
  ...
  def check_account(login, password)
    accounts.map { |account| { login: account.login, password: account.password } }.include?(login: login,
                                                                                             password: password)
  ...
  def validate_cards(answer)
    return Message.wrong_number unless answer&.to_i <= @current_account.cards.length && answer&.to_i.positive?
  ...
  def display_cards_destroy
    @current_account.cards.each_with_index do |card, index| Message.display_card_destroy(card[:number],
                                                                                         card[:type], index + 1)
    end
  end
end
